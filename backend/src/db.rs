...
const MIGRATIONS: &[(i64, &str)] = &[
    ...
    // 20/21 are reserved by branches in flight; this took the next free number rather
    // than a lower one, because `highest_applied` skips anything at or below the max
    // already recorded — a smaller version merged later would never run.
    (
        22,
